...
function buildMenu(): void {
  ...
  },
  {
    ...
    submenu: [
      ...
      {
        label: 'Show WBST in Gen 1',
        type: 'checkbox',
        checked: initSettings.showWbst === true,
        click: (menuItem) => {
          saveSetting('showWbst', menuItem.checked)
          mainWindow?.webContents.send('show-wbst-changed', menuItem.checked)
        }
      },
      {
        label: 'Show UBST in Gen 1',
        type: 'checkbox',
        checked: initSettings.showUbst === true,
        click: (menuItem) => {
          saveSetting('showUbst', menuItem.checked)
          mainWindow?.webContents.send('show-ubst-changed', menuItem.checked)
        }
      },
    ]
  },
...
ipcMain.handle('get-show-wbst', () => {
  const settings = loadSettings()
  return settings.showWbst === true
})

ipcMain.handle('get-show-ubst', () => {
  const settings = loadSettings()
  return settings.showUbst === true
})
