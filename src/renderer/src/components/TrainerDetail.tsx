...
function PartyCard({ pokemon, game, index }: PartyCardProps) {
  ...
  const handleExportCard = useCallback(async () => {
    ...
    setExporting(true)
    try {
      // The card uses bg-gray-800/50 on screen (translucent so it blends with the page).
      // For capture we swap to an opaque fill so the export isn't see-through.
      ...
      // set it as inline style on the root element, overriding the card's own bg.
      // Instead, fill a wrapper canvas with bgColor and composite the captured card on top
      // (same pattern as the team export below).
      ...
      await saveExportPng(out.toDataURL('image/png'), buildExportFilename(game, `${safeName}_Lv${pokemon.level}`))
    } catch (err) {
  ...
  return (
    <div ref={cardRef} className={`${exporting ? 'bg-gray-800' : 'bg-gray-800/50'} border border-gray-700/60 rounded-lg overflow-hidden flex flex-col relative group`}>
      <button
        data-export-ignore
      ...
      <div className="px-3 pb-2 pt-1">
        <div className="space-y-1">
          {statConfig.map(({ key, label, color }) => {
            ...
            const pct = Math.min((value / TRAINER_STAT_BAR_MAX) * 100, 100)
            return (
...
function TeamTypeSummary({ party, game, trainerName }: { party: TrainerPokemon[]; game: string; trainerName?: string }) {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      await saveExportPng(dataUrl, buildExportFilename(game, `${safeName}_type_effectiveness`))
    } catch (err) {
      console.error('Export failed:', err)
    } finally {
      setExporting(false)
...
export default function TrainerDetail({ trainerId, selectedGame }: Props) {
  ...
  const handleExportParty = useCallback(async () => {
    ...
    try {
      ...
      await saveExportPng(out.toDataURL('image/png'), buildExportFilename(selectedGame, `${safeName}_team`))
    } catch (err) {
      console.error('Export failed:', err)
    } finally {
