...
- Sprites are bundled locally (`src/renderer/public/sprites/{artwork,home}/<id>.webp`, downloaded by `node scripts/download-sprites.mjs`, which re-encodes PokeAPI's PNGs to WebP q90 with `sharp`, a dev-only dependency); base forms are keyed by national dex number. HOME sprites are only ever drawn at ≤44 CSS px, so the script also shrinks them from 512px to 128px; artwork stays full size for the lightbox and exports. Re-running it converts sprites left by older versions (.png, or full-size HOME) in place
